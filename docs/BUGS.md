...
- **D22. A COPY RULE ABOUT A SURFACE CANNOT BE APPLIED TO A STRING IN ISOLATION — READ THE RENDER SITE BEFORE EDITING THE WORDS.** "One sentence, one action" is a property of what the user SEES, and what the user sees is the string plus whatever the renderer puts beside it. `[1.6.5]` rewrote eight empty states to that rule and rewrote each one in the catalogue. `dash_no_active_goals` became "No active goals. Create one in Tasks." — and `dashGoalsHtml()` already appends a `dash_create_one_in_tasks` link reading "Create one in Tasks." on the same line. The rendered surface said it **twice**, once as prose and once as the control.

  **THE STRING WAS CHECKED AGAINST EVERYTHING EXCEPT THE THING THAT RENDERS IT.** It was checked against the gates (none assert its value), against the catalogue (well-formed, described, no em dash), and against the copy rule itself (one sentence, one action — true of the string, false of the surface). Every one of those checks passed, and the surface was still wrong. The entry's own translator description even recorded that R4 had split the sentence off the button, which is precisely the note that should have stopped it and did not, because nobody read it while editing the value above it.

  **THE CHECK IS ONE LINE OF WORK AND IT IS NOT OPTIONAL:** for each string, open its render site and say what the renderer emits immediately before and after it. Applied to the other seven afterwards it found no second instance, but it also showed that "correct" varies with the neighbour — `goal_no_tasks_yet` ("Add the first one below.") sits directly above a `+ Add task` button and is right precisely BECAUSE it describes the control rather than restating its label. Prose that names an adjacent control's own words is the defect; prose that points at it is the pattern.

  **Generalises to any rule stated about a surface** — an empty state, an error, a confirmation, a tooltip. The renderer is part of the sentence. Descriptions are also the enforcement point across languages: this one now tells the translator the link follows, so the call to action cannot be added back in another locale. Asana 1218038783461517.
...
### Section L: Whole-Object Write Clobbering

Run before adding, or writing a harness for, ANY code path that mutates the `data` storage key - background (`background.js`) or page (`newtab.js`). The background half is serialized (**L1**); the page half is not (**L5**), and the mechanism is identical.
...
- **L5. ON THE PAGE SIDE THERE IS NO QUEUE, SO A HANDLER MUST RE-READ AT THE POINT OF WRITE - 94 OF THEM DO NOT.** `saveAll` persists the **whole** `data` object with no merge, exactly as in **L1**; what differs is that `background.js` funnels its writers through `enqueueBgData` and **`newtab.js` has no page-side equivalent**. Instead the page keeps one long-lived module-level `data` (`newtab.js:4`) that every handler closes over, and `chrome.storage.onChanged` refreshes it - but **deliberately suppresses own-tab writes** (`__lastWrite.tab === TAB_INSTANCE_ID` plus `_pendingWriteIds`), because refreshing on your own write would blow away DOM state mid-interaction. That suppression is correct, and it is also the hole: after any write made from the page's own context, `data` is stale, and the next handler to mutate-and-save it writes the stale copy back over the top. No error is raised on either side.

  **The measured spread, taken 2026-09-03 by enumerating rather than sampling: of 94 call sites in `newtab.js` that pass the module-level `data` to one of the 80 exported writers, 94 close over it and 0 re-read at the point of write.** So this is not one handler's slip - closing over `data` is the house pattern, and every one of those sites is a latent clobber under the right interleaving. A first pass reported "3 re-read"; all three were false positives of a backward scan that found a `data = await Storage.getAll()` belonging to a **sibling branch**, where it was a post-write refresh rather than a pre-write re-read. Counting the right thing mattered more than counting carefully.

  **The demonstration, because a fix whose failure cannot be shown has not been shown to fix anything.** Render the Dashboard; from the page's own console create a task and save; then drive a pick through the control; then read storage. On `4217cb0` the pick persists and **the task created seconds earlier is gone** (task count 27, marker absent) - precisely the shape of the live report, where picks were "listed, then not". With the re-read in place both survive (count 28). The console origin is not incidental: a write from a *different* tab would have refreshed `data` through `onChanged`, and the bug would have hidden.

  **The rule for new code:** a page handler that mutates `data` and saves must begin with `data = await Storage.getAll()`. Only the three-picker paths carry it today (`newtab.js:1057`, `:1600`); the remaining sites are a known, unremediated class filed on its own task rather than patched blind, because re-reading reassigns the shared module snapshot and each site needs its own check that no render between the read and the write depends on the old one. `2026-09-03`, Asana 1218146737495265.
...
- **P20. AN ASSERTION THAT REQUIRES A LITERAL VALUE FORMAT BLOCKS THAT VALUE FROM EVER BEING TOKENISED — and it does not fail by reporting absence, it fails by SILENTLY MATCHING A DIFFERENT RULE.** Three gates in one round asserted CSS by its literal spelling — `padding-bottom: 48px`, `color: rgba(255, 255, 255`, an `rgba()` on `.tt-progress-pct-base` — so replacing the literal with a token that computes to the identical value broke all three, on a change that touched nothing any of them protects.

  **THE FAILURE MODE IS WORSE THAN A FALSE FAILURE.** A regex like `/\.tt-progress-pct-base \{[^}]*color:\s*(rgba?\([^)]*\))/` does not stop when the first matching rule loses its literal. It BACKTRACKS to a later rule with the same selector — here the `html.has-bg.bg-light` override — and compares that instead. The chip-ink gate therefore reported that two inks "sit on the same side of the range" while reading a rule nobody had touched. It was still a red gate, so it was caught; had the substituted rule happened to satisfy the assertion, it would have gone green over the wrong evidence. **That is Q2 arriving through the value format rather than through the selector**, and pill-clarity's own comment cites Q2 three lines above the assertion that fell to it.

  **THE FIX IS TO ASSERT THE VALUE, NOT ITS SPELLING.** today-cockpit's was rewritten to accept a literal OR a token and to RESOLVE the token against `:root` before comparing — which also made it strictly stronger, since it now catches a token silently redefined to a different value, something the literal check could never have done. The other two were left asserting literals and their declarations were kept as literals with a comment saying why, because changing them was out of that round's scope. **A declaration pinned as a literal by a gate is a debt, not a resting state**: it is invisible to every later token sweep and will be "tidied" by someone who does not know why it is there.

  Distinguish from its neighbours: **P16** is over-specification in a negative assertion (silent pass), **P17** is the `\b` trap (false failure). This one is a positive assertion whose over-specification makes an unrelated refactor impossible and, when it does fire, points at the wrong rule. `9634450`, Asana 1218038783461517.

- **P21. A PRESENCE ASSERTION IS NOT A VISIBILITY ASSERTION — `innerText.indexOf(s) !== -1` proves the string EXISTS, never that anyone can see it.** The `[1.6.5]` checkpoint captured the free-tier Tasks preview to photograph two rewritten empty states, and asserted both were present. Both were present. One sat **below the fold**, so the delivered frame did not show the state it existed to show, and the run reported green. A screenshot harness whose assertions cannot see the viewport is asserting about the DOM while claiming to be asserting about a picture.

  **THE FIX IS TO MEASURE THE BOX**: the element's `getBoundingClientRect()` must be non-zero, not `visibility:hidden` / `display:none`, and wholly inside the captured viewport. Anything less admits four different invisibles — below the fold, zero-sized, painted off to the side, and present-but-hidden — all of which read identically to a presence test.

  **THE SECOND-ORDER TRAP, WHICH BIT ON THE VERY NEXT RUN.** The first visibility check took the FIRST innermost element matching the text and measured that. "No active goals." is a **substring** of the Tasks tab's own "No active goals. Create your first goal.", which lives in a hidden panel earlier in the document, so the check measured a 0x0 box and failed a string that was plainly on screen. A false failure this time, and a false PASS the moment the order runs the other way. So a visibility check must **consider every match and pass if any one is painted**, and report all boxes so the choice is auditable.

  **This generalises to every screenshot harness the project writes**, and `[1.7.0]` and `[1.8.0]` rebuild two boards. Related but distinct: **P13**'s vacuous pass is an inspection set that collapsed to nothing; this one inspects exactly the right element and asks it the wrong question. Asana 1218038783461517.
