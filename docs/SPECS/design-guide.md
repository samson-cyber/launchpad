...
- **~~SETTLED 2026-09-07 ([1.8.5]) — the bottom-right void, after three deferrals.~~ CORRECTED 2026-09-09 ([1.8.6]). THE 2026-09-07 ENTRY WAS WRONG AND IS REPLACED, NOT QUIETLY EDITED.** What it said was: *"the ruling is that this is correct and it is not to be re-opened without new evidence"*, and *"the space is not a hole: it is the wallpaper"*. It was ruled unconditionally from a SINGLE shape of the `busy` fixture and it does not hold at a second.
- **The three measurements, all at 1440 on the same plain ground.** Goals rows against Due-today rows, then the gap between the bottom of the Goals CARD and the bottom of its equal-height grid CELL: **5 goals / 8 due → 277px** (the shape the original ruling was made from); **5 goals / 1 due → 16px**; **1 goal / 10 due → 488px, with an 80px card sitting at the top of a 568px column** (**RE-MEASURED 2026-09-15: 461px on a 541px column**, see below). At 16px the columns read as designed. At 277px the wallpaper argument holds. At 488px it does not — that reads as a stranded card, not as page, and the original entry asserted the opposite as a general rule.
- **What survives, narrowed to what was measured.** The 7/5 split and the content-height cards are correct, and the asymmetry behind them is still semantic: due-today is a working list capped at ten rows, goals a bounded set capped at five, so the left card is designed to be the deeper one. The three ways of closing a MODERATE gap are still worse than the gap — stretching the card puts air inside a frosted surface, moving a module presumes a spare one, and inventing a module to fill space is the failure this arc refused three times. **That reasoning is now scoped to the moderate case and carries no claim about extreme imbalance**, which is the case it was never tested against.
- **The extreme case is open, and it is a design decision rather than a defect to patch.** Filed rather than settled here. Note the measured ratio for whoever takes it: the Goals card occupies 92% of its cell at 16px, 40% at 277px, and 14% at 488px. No threshold is asserted between 14% and 40%, because the data does not locate one and inventing thresholds the data cannot justify is the specific failure this arc has refused repeatedly.
- The column is not structurally empty — it has a second slot and the evening uses it. Measured with the end-of-day boundary moved back: the Day Recap tile lands beneath the Goals card at 87px. That remains true, and at 488px it closes 87 of the gap and leaves 401.
- **RE-MEASURED 2026-09-15, before acting on the ruling, and the numbers have MOVED SLIGHTLY while the finding has not.** Same three shapes, same method, at 1440 on a plain ground: **5 goals / 8 due → 277px** and **5 goals / 1 due → 16px** are BYTE-IDENTICAL to the 2026-09-09 figures, and the extreme reads **461px on a 541px column** rather than 488 on 568 — 27px less, with the Goals card still exactly 80px, so the ratio moves 14% to 15%. The Dashboard has changed underneath this entry since it was written (the greeting line, the hero ring rendering once a focus target exists, two rounds of accent-fill work), so a 27px drift in the tallest column is expected; what matters is that the SHAPE is unchanged — an 80px card stranded atop a column seven times its height. The ruling below therefore stands on its evidence. The Day Recap's 87px was NOT re-measured this pass; if it still holds it now closes 87 of 461 and leaves 374.
- **RULED 2026-09-12 (Samson): FILL THE COLUMN, DO NOT LOWER THE CAP.** Option (a), lowering DASH_DUE_MAX from ten, is rejected: the due list is the board's ACTIONABLE content and goals are context, so reducing the primary content to close a gap in the secondary column is backwards — and the shape where it bites is a user with little structure and a lot due, which is exactly the day on which hiding four items costs most. The void waits for a module rather than being solved by subtraction, so this entry stays OPEN by design and DASH_DUE_MAX stays at ten. Asana 1218303707651534.
...
- **CORRECTION 2026-09-07 ([1.8.5]).** This line read *"the range selector is the first row and the only control on the board"*. It was written before the CSV export existed. Row one now carries **the range and the actions that operate on it** — the selector and the export control. The export acts on exactly the range that row selects, and moving a control away from the thing it operates on to satisfy a sentence is the wrong way round. Its horizon caption still derives from the retention constant (D16).
- Row one is the range and the actions on it. Both of its text nodes sit directly on the wallpaper with no card behind them, so both carry the text shadow this guide requires for text on wallpaper; a **third** node there would mean the row wants a surface instead.
- **Hero (span 12).** Period total at `--display-1`, the aside figures beside it at `--display-3`, the bar chart beneath at full width. **CORRECTION 2026-09-07 ([1.8.5]):** these read `--fs-display-1` / `--fs-display-3`; no such tokens exist. The stylesheet defines `--display-1` and `--display-3`, and `--display-1` is asserted to appear exactly once, on `.dash-hero-num`, which Insights BORROWS rather than redeclaring. **Second correction:** the beside-figure is *best day*, not *longest session* — no reader for a stretch COUNT exists and inventing a threshold was refused in [1.8.3]. Bars are the accent at 0.85 alpha, today's bar at full; axis captions in secondary ink and tabular numerals; no gridlines, one baseline hairline.
...
- **Row three:** the heatmap (span 8) uses the accent at five alpha steps — **confirmed 2026-09-07 ([1.8.5]) by measurement rather than inherited:** at span 8 of a 1374 board its 24 columns land at 31.7px cells, and the weekly card demonstrably carries six rows at span 4. The heatmap EXCLUDES any day whose recorded hours do not account for that day's total, and captions how many days it is showing, because painting such a day would assert an empty morning the data cannot support; the weekly review card (span 4) is the only place on Insights with a comparison arrow, and the arrow is neutral ink, not green or red (a week with less focused time is information, not a fault).
