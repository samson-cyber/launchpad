...
- **The three measurements, all at 1440 on the same plain ground.** Goals rows against Due-today rows, then the gap between the bottom of the Goals CARD and the bottom of its equal-height grid CELL: **5 goals / 8 due → 277px** (the shape the original ruling was made from); **5 goals / 1 due → 16px**; **1 goal / 10 due → 488px, with an 80px card sitting at the top of a 568px column**. At 16px the columns read as designed. At 277px the wallpaper argument holds. At 488px it does not — that reads as a stranded card, not as page, and the original entry asserted the opposite as a general rule.
