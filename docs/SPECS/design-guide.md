...
- **SETTLED 2026-09-07 ([1.8.5]) — the bottom-right void, after three deferrals.** Measured on the `busy` fixture at 1440: the two row-two grid cells are equal-height siblings (both 465px), but the CARDS inside them are content-height, so the Goals card ends 261px above the Due-today card and that 261px is bare wallpaper. **The ruling is that this is correct and it is not to be re-opened without new evidence.** The asymmetry is semantic and deliberate: due-today is an unbounded working list capped at ten rows, goals are a bounded small set capped at five, so the left card is DESIGNED to be the deeper one, and a user with four goals has four goals. The three ways to close the gap are all worse than the gap. Stretching the Goals card to its cell puts ~260px of air inside a frosted card, and air inside a card reads as broken where air beside one reads as page. Moving a module means there is a spare module, and there is not. Inventing one to fill the space is the failure this arc refused three separate times, most recently in [1.8.3] when a stretch COUNT was declined for want of a threshold the data could not justify. And the space is not a hole: it is the wallpaper, which is this product's primary visual surface and the reason the card layer is translucent at all.
- The column is not structurally empty either — it already has a second slot, and the evening uses it. Measured in the same pass with the end-of-day boundary moved back: the Day Recap tile lands beneath the Goals card at 87px tall, leaving ~165px. So the shape is *one tall working column beside one short reference column that gains a tile at dusk*, which is a description of the day, not a defect in the grid.
