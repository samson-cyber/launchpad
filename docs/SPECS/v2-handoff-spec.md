...
### The pill — V2-Pill
Changes: a small tile at 16px radius in the hero tint. Six states as drawn: empty, card, slim, running (conic ring in `action`), paused (ring and chip in `paused` amber), the countdown (WM.5). The WORK chip in the action tint rides the session. The hide option: ⌄ on the card head collapses to slim; a second tap hides to a 28px `action` dot at the corner; a click on the dot restores.
