...
4. ~~**Soft cap / warning on recurring task instance accumulation**~~ — **RESOLVED, and it was resolved a long time before it was written down here.** The question asked whether a 60-day absence should generate 60 daily instances and proposed "max 7 overdue instances kept, older skipped silently". `[1.0.14]` built exactly that — `RECUR_OVERDUE_CEILING = 7` in `storage.js`, applied to **every** recurring template, not only habits — and this line was simply never updated, so `[1.14.4]` found a shipped answer sitting under an open question. **The number is unchanged: seven, because a week is the unit a person can read.** What `[1.14.4]` changed is the word *silently*: the sweep now records what it refused (`data.recurringCatchUp`, and `template.skippedDays` on a habit) and the Tasks tab says so once, dismissibly — "Picked up where you left off", never a count of days away. A ceiling that never mentions itself is the product deciding on the user's behalf in silence; a scold about the absence is what the celebration doctrine (2026-07-20) bans. The notice is the narrow thing between them.
