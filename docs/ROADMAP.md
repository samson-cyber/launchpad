...
- **RELEASE STATE as of 2026-09-01 — stated here in full, deliberately, rather than by pointer.** **`2.1.0` is SUBMITTED to the Chrome Web Store and AWAITING REVIEW** (uploaded 2026-09-01). **`2.0.0` is still LIVE**, is what users are actually running, and therefore remains the **store baseline** any permission diff is taken against, until review clears. The manifest reads **`2.1.0`**. **Submitted is not published:** nobody has 2.1.0 until Google approves it.
...
- **This block and CLAUDE.md's release-state block are REDUNDANT ON PURPOSE.** Neither defers to the other. Two independent assertions of the same fact can be checked against each other and disagree loudly; a pointer cannot disagree with anything. **These two files drifting apart is precisely what produced the 2026-08-30 error recorded above** — the correction landed in one and propagated into the other unexamined. Update both, in the same commit, or the pair stops being a check.
