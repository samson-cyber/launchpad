...
- **`preview-tasks.png`** — a demo task board with a fake running timer (*"active · 00:23:15"*) **beside the user's real notes** ("Call the studio back about the October shoot"). The banner sits above both, so nothing separates the real column from the demo one.
