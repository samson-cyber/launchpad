...
## RELEASE STATE — as of 2026-09-15

**THREE VERSIONS MATTER AT ONCE NOW, AND COLLAPSING ANY TWO OF THEM INTO ONE SENTENCE IS THE ERROR THIS BLOCK EXISTS TO PREVENT.** Read all three lines before writing anything about what shipped.

- **`2.2.0` is BUILT and AWAITING UPLOAD.** Cut 2026-09-15. **Built is not submitted.** The artifact exists and is tagged; nobody has uploaded it, so it is not even in review. Do not call 2.2.0 submitted, shipped or live.
- **`2.1.0` is SUBMITTED and AWAITING REVIEW, unchanged and UNCONFIRMED.** Uploaded 2026-09-01. **Submitted is not published.** Nothing in the 2.2.0 round could check the store dashboard — that needs Samson's account — so this line is exactly as stale as it was, and **it needs his confirmation before anyone acts on it.** If 2.1.0 has since been approved, 2.0.0 below is wrong and 2.1.0 is live.
- **`2.0.0` is the LIVE store version** as far as this repo can prove, and stays so until 2.1.0 is confirmed approved. It is therefore still the **store baseline** for a permission diff, because it is what users would be upgrading *from*.

**WHAT HAPPENS TO 2.1.0 WHEN 2.2.0 IS UPLOADED, because it is the question the upload turns on.** A Chrome Web Store item has ONE pending submission. Uploading 2.2.0 while 2.1.0 is still in review **REPLACES the pending item** — 2.1.0 is superseded and never reaches anyone, and the review clock restarts on 2.2.0. That is not a loss: 2.2.0 is a strict superset of 2.1.0, 132 commits on top of it. But it means **2.1.0 will never be a version any user ran** if it has not already been approved, and any note claiming otherwise would be wrong.

Supporting facts for 2.2.0:

- **The manifest is at `2.2.0`**, bumped at the cut. It is the ONLY version literal in shipped code — verified by grep across every `.js`, `.html`, `.css` and `.json` that ships.
- **Artifact:** built from the commit `v2.2.0` peels to, **939914 bytes**, sha256 `1638fb64f09f1d36779fff945ddb46c5a684f1cb3dfd485c2de0bf291a94068f`. **The permission diff against packaged 2.1.0 is EMPTY** — all five fields (`permissions`, `optional_permissions`, `host_permissions`, `optional_host_permissions`, `content_security_policy`) compare identical, taken from the ARTIFACT's own manifest rather than the tree's.
- **THE ARTIFACT SHA256 IS CHECKOUT-DEPENDENT, and this cost a re-tag — record it before the next release.** The zip embeds each entry's FILE MTIME, and a fresh `git worktree` stamps every file with the moment it was checked out. So the same commit built in a worktree and in the main checkout gives **the same byte count and the same CRC for every entry, and a DIFFERENT sha256**. Proven: two consecutive builds in one checkout are byte-identical (34/34 entries, zero timestamp differences); the worktree build of the same commit was not. **"Reproducible from the commit" means the CODE reproduces, never the bytes.** Build the artifact you intend to upload in the MAIN checkout, and hash that one.
- **Tag:** **`v2.2.0`**, annotated, on the build commit, carrying that sha256 in its message. **The commit‑hash pairing is deliberately not written here**, unlike the tags below, for the one reason it cannot be: the tag is on the commit that introduces this very text, and no commit can contain its own hash. Resolve it with `git rev-list -n 1 v2.2.0`.
- **THE BUILD CAUGHT A REAL SHIPPING DEFECT, on the first build in 132 commits.** `importers.js` was referenced by `newtab.html` and **missing from `build.sh`'s allowlist**, so the zip would have shipped a dead `<script src>` and every import path broken — while dev kept working, because the unpacked tree has the file. Exactly the class the `[1.9.4]` comment twelve lines above it in `build.sh` already records. Fixed in the build commit. **This is the argument for building more often than once per release.**

Supporting facts for 2.1.0:

- **The manifest read `2.1.0` at that submission.** That number matched a real submission — but that is timing, not a rule. See the doctrine below before using it as evidence of anything.
...
**HOW RELEASE STATE IS DETERMINED: THE ANNOTATED TAGS, NEVER THE MANIFEST.** A present `v<version>` tag means that version was submitted; an absent one means it was not. `v1.0.5` (`3eb9323`), `v2.0.0` (`92eeb68`) and `v2.1.0` (`15797ad`) all exist, and `v2.2.0` joins them at the 2026-09-15 cut; there is no `v2.0.1`, correctly. **A manifest bump is never evidence that a release occurred** — it records what the next submission would carry, not what users have. `2.0.1` is the standing counter-example: the manifest read `2.0.1` for three days with no release behind it. Note also what a tag does *not* tell you: it records **submission**, not approval. `v2.1.0` existing means 2.1.0 was uploaded, not that it is live.
...
**THE NEXT STORE SUBMISSION IS `2.2.0`, AND IT IS BUILT BUT NOT UPLOADED** — the manifest was bumped at the cut, which is the only moment it may be. The submission AFTER that one has no number yet and must not get one until it is cut. Its store baseline will be whatever is live at that moment — `2.0.0` today, `2.1.0` once approved. Its annotated tag is applied at submission, and **its artifact is built fresh from the commit being submitted**. An older zip is never resurrected: master has moved since every previous build, so any surviving artifact would ship code that no longer matches the tree it was cut from.
...
- **A WORKTREE SESSION PUSHES `HEAD:master`; THE MAIN CHECKOUT DOES NOT FOLLOW.** `master` is checked out in `C:\Dev\Git\launchpad`, so a session working in `.claude/worktrees/*` cannot move that branch and pushes its commit straight to `origin/master` instead. The push succeeds, `origin/master` is correct, and the main checkout's local `master` silently stays where it was - six commits behind by the end of 2026-09-17, which is how far one day of worktree rounds can carry it. **Run `git pull` before working in the main checkout**, and read an ahead-by count there as a question rather than an answer.
...
**THE `[N.N.N]` MARKERS ARE ARC NUMBERS, NOT PRODUCT VERSIONS. THIS IS THE THING THE BOARD MAKES EASY TO GET WRONG.** The 2026-09-12 board cleanup found `[1.0.x]`, `[1.5.0]`, `[1.6.0]`–`[1.11.0]`, `[2.0]` and `[2.1.0]` all coexisting, which reads as though the product is building 1.11 while the store sits on 2.1 — and as though `[1.5.0]` and `[1.11.0]` are somehow in progress at the same time. Neither reading is right, and nothing is wrong with the versioning.

- **A marker is a planning ID for one arc of work.** `[1.11.0]` is the **eleventh arc**, not version 1.11. `[1.5.0]` (Localization) and `[1.11.0]` (Launcher data) being open together is two arcs in flight, not two versions in flight — exactly as `[1.5.0]` paused at its R4/R5 boundary while later arcs ran.
- **They were coined before `2.0.0` shipped and were never renumbered.** The whole `[1.x.y]` series predates the Pro-era store line. When the store jumped to `2.x`, the markers stayed where they were.
- **THE PRODUCT HAS SHIPPED EXACTLY THREE VERSIONS TO THE CHROME WEB STORE: `1.0.5`, `2.0.0`, `2.1.0`.** Nothing was skipped, nothing is missing, and no marker corresponds to any of them. (`1.0.0`–`1.0.4` shipped before tagging began; see the tag notes below.)
- **The authoritative record of what shipped is the ANNOTATED GIT TAGS**, never the markers and never the manifest. `git tag` returns `v1.0.5`, `v2.0.0`, `v2.1.0`, `v2.2.0` and nothing else. A marker tells you which arc a commit belonged to; a tag tells you a user can install it.

**WHERE MARKERS ACTUALLY APPEAR, measured across the log rather than assumed.** Of 423 commits, 154 carry a subject marker and 269 do not. The `[1.0.x]`–`[1.5.x]` arcs put the marker in the **subject**; the `[1.6.0]`–`[1.11.0]` arcs put it in the **body** and in the **Asana task title** instead, and appear in subjects essentially never (one `[1.10.0]` chore across six arcs). `[tooling]` and `[site]`, which the board uses, appear in **no** commit subject at all — they are Asana-side labels. The log also carries non-numeric markers the convention above does not mention (`[Tasks]`, `[Trash]`, `[Experience]`, `[Dev Tooling]`, `[R3]`), and three spellings of one arc (`[2.1]`, `[2.1.0]`, `[2.1.1]`). **This is recorded, not corrected.**

**DO NOT RENUMBER ANYTHING TO TIDY THIS.** Renaming eleven arcs across `DECISIONS.md`, `BUGS.md`, 154 commit subjects and every Asana task is a great deal of churn for a naming collision, and the history would then disagree with itself — a `DECISIONS.md` entry citing `[1.10.11]` would point at a marker no commit carries. The explanation above is the cheaper and more honest fix. Read a marker as "which arc", read a tag as "what shipped", and the clash stops mattering.
...
- **Where the record lives, and why closing a task is safe.** Asana holds the **round-by-round history** — every PLAN, IMPLEMENTATION and REVIEW comment, with the measurements and the reasoning behind each round. A **completed Asana task keeps all of it and stays searchable**; closing a task retires it from the board, it does not delete anything. `docs/` is authoritative for anything a future session must **act on** — the conventions, the decisions, the audit checklist, the release state. That split is what makes closing safe: if a fact has to change behaviour later it belongs in `docs/`, and if it is the story of how a round reached its answer it belongs in the Asana thread. When the two disagree, `docs/` wins and the Asana comment is history.
- **Session resumption — the Session Start Checklist at the foot of this file, plus the session-start prompts in `docs/ASANA.md`.** There is no handover document, and **there deliberately is no `docs/HANDOVER.md`** — this line pointed at one until 2026-09-01 and the file had never existed, so every session that followed it found nothing. Resumption is not a stored summary: it is **queried fresh** (docs, then the Asana board, then `git log`), because a stored one drifts from the canonical sources while still reading as current. Asana task `1214275087201909` holds that standing kickoff *procedure* for Claude Chat, and its own Anti-patterns section is the reason this is not a document: *"Do NOT update this task at end of session… Updating it would create a parallel summary that drifts from canonical sources."* Handover between Claude Chat sessions is a **chat-side** concern; Asana is the output log, per `docs/ASANA.md`, whose comment taxonomy is PLAN / IMPLEMENTATION / REVIEW / RESOLVED and nothing else.
